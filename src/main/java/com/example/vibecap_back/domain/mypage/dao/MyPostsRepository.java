...
public interface MyPostsRepository extends JpaRepository<Post, Long> {
    @Query("select new com.example.vibecap_back.domain.mypage.dto.response.GetMyPostsResponse(p.postId, p.vibe.vibeId, v.vibeImage) " +
            "from Post p left join Vibe v on p.member.memberId = :memberId where p.vibe.vibeId = v.vibeId order by p.postId desc")
