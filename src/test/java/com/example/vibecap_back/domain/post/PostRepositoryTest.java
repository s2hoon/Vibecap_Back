    ...
    void insertAndSelectDummies() {
        try {
            ...
            for (int i=1; i<TEST_NUM+1; i++) {
                Vibe dummyVibe = VibeFactory.getVibe(member.getMemberId(), i);
                Vibe savedVibe = vibeRepository.save(dummyVibe);
                Post dummyPost = PostFactory.getPost(member, savedVibe, i);
                Post savedPost = postsRepository.save(dummyPost);
                // then : 저장된 post 조회
                assertThat(savedPost).isEqualTo(PostFactory.select(member, savedVibe, i));
            }
        } catch (Exception e) {
